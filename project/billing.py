...
class BillClass:
    def __init__(self, root):
        ...
        ProductFrame1.place(x=6, y=110, width=390, height=550)
        ...
        CustomerFrame.place(x=400, y=110, width=510, height=70)
        ...
        txt_contact = Entry(CustomerFrame, textvariable=self.var_contact, font=("times new roman", 13), bg="lightyellow").place(x=380, y=35, width=120)

        # Calculator and Cart Frame
        Cal_cart_Frame = Frame(self.root, bd=2, relief=RIDGE, bg="white")
        Cal_cart_Frame.place(x=400, y=190, width=510, height=360)

        # Calculator
        self.var_cal_input = StringVar()
        Cal_Frame = Frame(Cal_cart_Frame, bd=9, relief=RIDGE, bg="white")
        Cal_Frame.place(x=5, y=10, width=265, height=340)
        ...
        Add_CartWidgetsFrame.place(x=400, y=550, width=510, height=110)
        ...
        btn_add_cart = Button(Add_CartWidgetsFrame, text="Add | Update Cart", command=self.add_update_cart, font=("times new roman", 15, "bold"), bg="orange", cursor="hand2").place(x=340, y=70, width=160, height=30)

        # Billing Area
        billFrame = Frame(self.root, bd=2, relief=RIDGE, bg="white")
        billFrame.place(x=914, y=110, width=430, height=410)
        ...
        billMenuFrame.place(x=914, y=520, width=430, height=140)
        ...
        btn_generate = Button(billMenuFrame, text='Generate Bill', command=self.generate_bill, font=("goudy old style", 15, "bold"), bg="#009688", fg="white", cursor="hand2")
        btn_generate.place(x=246, y=80, width=160, height=50)

        # Footer
        lbl_footer = Label(self.root, text="IMS-Inventory Management System | Developed By Akash Jadhav\nFor any Technical Issue Contact: 7057320660", font=("times new roman", 12), bg="#4d636d", fg="white").pack(side=BOTTOM, fill=X)
        self.show()
...
# =============================All Functions==============================
    ...
    def search(self):
        ...
        cur = con.cursor()
        try:
            cur.execute(f"SELECT pid, name, price, qty, status FROM product WHERE name LIKE '%{self.var_search.get()}%' AND status='Active'")
            rows = cur.fetchall()
            if len(rows) != 0:
                self.ProductTable.delete(*self.ProductTable.get_children())
                for row in rows:
    ...
    def generate_bill(self):
        if self.var_cname.get() == '' or self.var_contact.get() == '':
            messagebox.showerror('Error', f"Customer Details are required")
    ...
    def bill_top(self):
        ...
        bill_top_temp = f'''
\t\tXYZ-Inventory
\t Phone No. 123456789 , XYZ City-313003
{str("="*47)}
    ...
    def bill_middle(self):
        ...
        try:
            for row in self.cart_list:
                ...
                qty = int(row[3])
                price = float(row[2])
                self.txt_bill_area.insert(END, "\n " + name + "\t\t\t" + str(qty) + "\t\tRs." + str(price * qty))
                # Update Product Quantity in the Database.
                cur.execute('UPDATE product SET qty = qty - ? WHERE pid=?', (qty, pid))
                con.commit()
            con.close()
            self.show()
        except Exception as ex:
